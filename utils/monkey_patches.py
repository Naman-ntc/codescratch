...
try:
    from flash_attn.flash_attn_interface import flash_attn_func, flash_attn_unpadded_qkvpacked_func
...
def bigcode_xformer_new_attn(self, query, key, value, attention_mask=None, head_mask=None):
    ...
    value = torch.repeat_interleave(
        value, self.num_heads//value.shape[2], dim=2)
    attn_output = xops.memory_efficient_attention(
...
def bigcode_xformer_new_attn(
    ...
    attn_output = attn_output.reshape(
        attn_output.shape[0], attn_output.shape[1], self.num_heads *
        self.head_dim
    )
    return attn_output, None
